...
jQuery.extend({
	...
	TN_checkName: function(name) {
		...
		if(name == ''){
			...
			var expression = new RegExp("^[A-Za-z0-9_\\-\\u4e00-\\u9fa5]+$");
	...
	//3: 密码长度只能在6-20位字符之间
	//4: 姓名只能由中文、英文、数字及“_”、“-”组成
	TN_checkPwd: function(pwd) {
		pwd = pwd.replace(/(^\s*)/g, "");//去除开头空格
		if(pwd == ''){
			return 2;
		}else if(this.TN_strlen(pwd)>20 || this.TN_strlen(pwd)<6){
			return 3;
		}else {
			var expression = new RegExp("^.*[A-Za-z0-9\\w_-]+.*$");
