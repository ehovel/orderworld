...
class PageClass
{
	...
	function __construct($myde_count=0, $pagesize=1, $myde_page=1,$page_url)//构造函数
	{	
		
		$this -> myde_count = $this -> numeric($myde_count);
		$this -> pagesize  = $this -> numeric($pagesize);
		$this -> myde_page  = $this -> numeric($myde_page);
		$this -> page_limit = ($this -> myde_page * $this -> pagesize) - $this -> pagesize;
		...
		$this -> myde_page_count  = ceil($this -> myde_count/$this -> pagesize);
